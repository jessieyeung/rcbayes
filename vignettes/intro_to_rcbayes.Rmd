...
##### Checking Convergence of the Model
When fitting models in a Bayesian framework, one cannot simply run the model and use the results without further inspection. It is always necessary to assess the model results to ensure tha the model has converged. In this case, convergence would imply that the model has converged to a particular target distribution, which is a necessary condition before you move on and use the model's results. 

One measure to check for convergence is to look at the potential scale reduction statistic, commonly referred to as the R-hat statistic [@gelman1992inference]. Ideally, you want to see the R-hat values close to 1. R-hat values greater than 1 indicate that convergence has not been achieved. Generally, an R-hat values less than 1.05 are a good sign, although there is not fixed threshold.

Another measure to check convergence is the effective sample size of the data. The larger the effective sample size the better. The effective sample size is too small if it is less than around 1% of the number of iterations. (The number of iterations is defaulted to 1000 post-warmup iterations * 4 chains = 4000, unless changed.) Refer to this [R documentation](https://www.rdocumentation.org/packages/popkin/versions/1.3.0/topics/n_eff) for more details. 

The `check_converge` object in `rc_res` allows you to check the R-hat values and effective sample size. 

```{r}
rc_res[['check_converge']]
```

In this example, the R-hat values are all close to 1 and effective sample sizes are sufficiently large. 

To fix a non-convergent model, please see the `rcbayes` vignette *Dealing with Model Convergence when Using mig_estimate_rc*.

##### Examining Model Results

The resulting `rc_res` object is shown below. The `pars_df` shows the median estimate and lower and upper bound of a 95% credible interval for the Rogers-Castro parameters. In this example, the working age peak was estimated to be at 24.9 years (95% CI: [23.1, 26.8]).
...
When running `mc_estimate_rc` it is common to see warnings from Stan, particularly when the retirement and post-retirement families are included in the model. Warnings about divergent transitions, low ESS and maximum treedepth can often be eliminated by changing the Stan settings as follows:  

* setting initial values `init` stategically using the `rcbayes::init_rc` function 
* changing the Stan settings to have a larger `adapt_delta`, `iter` and `maximum_treedepth`, respectively. 

For a more in-depth example of dealing with convergence issues and how to use the `init` function, please see the `rcbayes` vignette *Dealing with Model Convergence when Using mig_estimate_rc*. 
