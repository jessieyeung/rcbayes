...
That is, users have an option to input their data as counts or as rates. Depending on which one is used, a slightly different model will be run. If the user provides data as counts using `migrants` and `pop`, a Poisson model will be applied. If the user provides data as rates, a Normal model will be applied. 
